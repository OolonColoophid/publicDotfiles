...
#+BEGIN_SRC elisp
  (defun my/tangle-auto ()
    "If the current file is in specified directories, the code blocks are tangled"
    ...
    (when (equal (file-name-directory (directory-file-name buffer-file-name))
                 (concat (getenv "HOME") "/Dropbox/Documents/Programming/python/pythonProgrammingBeginnerBook/chapter3"))
      (org-babel-tangle)
      (message "%s tangled" buffer-file-name))

    )

  (add-hook 'after-save-hook #'my/tangle-auto)
#+END_SRC
