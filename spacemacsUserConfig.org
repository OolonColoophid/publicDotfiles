...
**** TODO Fix ~org-agenda-custom-commands~ for 'everything'
This is not currently producing an .ics. It did until 22 Feb. I've gone back to an earlier working version of the code, but running it still doesn't work. This suggests the code is fine. What's changed? The .org files in the agenda, presumably; one of aspect of these is causing the output to fail.

Note that the 'anniversairies' custom command does work.

Solution probably involves rebuilding the 'everything' code from scratch.

*** Automatic Export

I have one Emacs system that I'd like to be doing this (Archipelago). This should be after 2160 seconds, or 36 mins of idle time.
